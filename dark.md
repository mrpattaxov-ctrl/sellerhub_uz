...
| 2026-05-21 | FBS sahifa qayta yozildi: swagger ko'rib endpoint `GET /v2/fbs/orders` aniqlandi. Status enum 6 ta'dan 11 ta'ga kengaytirildi (haqiqiy enum: CREATED, PACKING, PENDING_DELIVERY, DELIVERING, DELIVERED, ACCEPTED_AT_DP, DELIVERED_TO_CUSTOMER_DELIVERY_POINT, COMPLETED, CANCELED, PENDING_CANCELLATION, RETURNED). Klikli qatorlar `/fbs/<id>` ga olib boradi. Yangi sahifa `templates/fbs_order_detail.html` (placeholder — endpoint kutilmoqda). Status chip qatori (`.fbs-status-chip` light + dark). |
| 2026-05-21 | Bosqich 1 tugadi: `GET /v2/fbs/orders/count` (count chip badge'lar) va `GET /v1/fbs/order/{orderId}` (detail sahifa) endpointlari ulandi. `templates/fbs_order_detail.html` to'liq qayta yozildi: kartalar (Mijoz / Sanalar / Yetkazib berish / Ombor / Mahsulotlar) `.fbs-detail-card` klassi bilan. Mahsulotlar jadvali rasmlar bilan (`.fbs-items img`). Dark mode 100% mos. 11 ta status badge ranglari `[data-bs-theme="dark"]` da palitra'ga mos qilib qayta tuzildi. |
| 2026-05-21 | Bosqich 2 — FBS action tugmalari. `templates/fbs_order_detail.html` ga action bar, cancel modal, IMEI modal va toast container qo'shildi. Yangi klasslar: `.fbs-actions-bar`, `.fbs-action-btn-primary/success/warning/danger`, `.fbs-action-empty`, `.fbs-imei-item`, `.fbs-toast-container`, `.fbs-toast`, `.fbs-toast-success`, `.fbs-toast-error`. Dark mode overridelar palitra'ga moslangan (surface `#1a1d27`, border `#2d3148`, accent ranglari yorug'roq variantlar — `#3b82f6`/`#10b981`/`#8b5cf6`/`#ef4444`). Modal va dropdownlar Bootstrap'ning standart `[data-bs-theme="dark"]` mexanizmi orqali to'g'ri rang oladi. |
| 2026-05-21 | Bosqich 3 — DBS action tugmalari. Mavjud `.fbs-actions-bar` ichida scheme="DBS" buyurtmalar uchun alohida workflow: PACKING→"Yetkazishga olaman" (primary), DELIVERING→"Yetkazib berildi" (success) + `.fbs-dbs-code-input` (Bootstrap `.form-control`, dark mode standart), COMPLETED→"Qaytarma yaratish" (danger). Yangi CSS klassi YO'Q — barcha ranglar mavjud `.fbs-action-btn-*` palitra'sini qayta ishlatadi, shu sababli dark mode avtomatik mos keladi. |
| 2026-05-22 | Bosqich 4d — Yangilash UX + last-synced indikator. `templates/fbs_orders.html` ga yangi klasslar: `.fbs-list-toast-container`, `.fbs-list-toast`, `.fbs-list-toast.is-success`, `.fbs-list-toast.is-error`, `.fbs-last-synced`. Yangilash tugmasiga `#fbsReload .spinner-border-sm` + `.is-loading` state. Dark mode override'lar palitra'ga moslangan (toast success `#052e23/#6ee7b7`, error `#2a0e0e/#fca5a5`, last-synced text `#64748b`). |
| 2026-05-22 | Bosqich 4e — performance fix (UI o'zgarmadi, dark mode'ga ta'sir yo'q): worker parallel fetch (`app.py::_fbs_sync_one_shop` — 8 active + 3 windowed status `ThreadPoolExecutor` bilan parallel), Yangilash tugmasi faqat joriy status'ni Uzum'ga uradi (`templates/fbs_orders.html` — `loadCounts(false)` count chip'lar DB'dan keladi). COMPLETED/CANCELED/RETURNED window 30 kun qoldi (foydalanuvchi tarixning shrinkage'iga rozi bo'lmadi). Natija: Yangilash bosish ~24s → ~3s. |
| 2026-05-22 | Bosqich 4f — sync strategy: DELETE+INSERT → UPSERT (UI o'zgarmadi). `core/fbs_sync.py::upsert_orders` (yangi, `INSERT ... ON CONFLICT (order_id) DO UPDATE`). Worker (`app.py::_fbs_sync_one_shop`) endi 11 ta status'ni (8 active + 3 terminal) `date_from` filtersiz parallel tortadi va UPSERT qiladi. Tarix kumulyativ to'planadi — yangi do'kon attach qilinganda birinchi tick to'liq backfill bo'lib ishlaydi. `core/fbs_data.py::_refresh_shop_status` ham UPSERT'ga o'tdi. Eski `_FBS_ACTIVE_SYNC_STATUSES` / `_FBS_WINDOWED_SYNC_STATUSES` / `_FBS_WINDOWED_DAYS_BACK` o'rniga `_FBS_ALL_SYNC_STATUSES` (11). |
