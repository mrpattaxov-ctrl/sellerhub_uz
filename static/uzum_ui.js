(function () {
  ...
  function initUzumUI() {
    ...
    const SL = _SIDEBAR_LABELS[lang] || _SIDEBAR_LABELS.ru;
    const subscriptionHref = document.body.dataset.subscriptionHref || "/subscription";
    const subscriptionLabel = document.body.dataset.subscriptionLabel || (isAdmin ? SL.subscriptions : SL.subscription);
    ...
    links.forEach(l => {
      ...
      // FBS/DBS is one section spanning both /fbs and /fbs/invoices —
      // highlight the parent link for any child path under /fbs/.
      const isFbsSection = l.href === "/fbs" && (path === "/fbs" || path.startsWith("/fbs/"));
      if (path === l.href || (l.href === "/groups" && path === "/") || isFbsSection) {
  ...
  // Browser-side auto-sync REMOVED 2026-05-24.
  // The old setInterval(10min) fired a full product sync for EVERY shop
  // sequentially, then called window.location.reload() — which hung the
  // tab for 30–50s every 10 min and lost the user's place mid-task.
  //
  // It's a duplicate of the server-side background workers anyway:
  //   * Products: `OpenAPISync` loop runs in app.py on its own cadence
  //   * FBS orders: `_fbs_sync_loop` runs every 10 min via _fbs_sync_tick
  // So the data stays fresh without browser participation. If a user
  // wants a manual refresh, the per-page "Yangilash" button still works.
})();
